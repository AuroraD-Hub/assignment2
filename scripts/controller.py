...
class Control(object):
	...
	*/state/position* topic.
	...
	def execute(self, goal):
		...
		#pos_x = 1.5
		#pos_y = 8.0
		pos_x = client.call('ADD','DATAPROP','IND',['hasX', map_info.room])
		pos_x = pos_x.queried_objects
		pos_x = float(pos_x[0][1:4])
		pos_y = client.call('ADD','DATAPROP','IND',['hasY', map_info.room])
	...
	def move_to(self, pos_x, pos_y, mb_target, move_client, goal):
		...
		rospy.loginfo(pnm.tag_log(f'status is {move_client.get_state()}', LOG_TAG))
	...
	def room_reached(self, goal):
		...
		previous_pos = previous_pos.queried_objects
