...
class Control(object):
	...
	def __init__(self):
		...
		self.pubX = rospy.Publisher(f'{pnm.TOPIC_POSITION}/x', Float64, queue_size=1, latch=True)
		self.pubY = rospy.Publisher(f'{pnm.TOPIC_POSITION}/y', Float64, queue_size=1, latch=True)
	...
	def execute(self, goal):
		...
		self.pubX.publish(pos_x)
		self.pubY.publish(pos_y)
	...
	def move_to(self, pos_x, pos_y, goal):
		...
		if (abs(self.x_curr-pos_x)<1) and (abs(self.y_curr-pos_y)<1): # errore
			self.room_reached(goal)
		else:
			rospy.loginfo(pnm.tag_log(f'Something went wrong... Point reached is {self.move_client.get_result()} instead of [{self.pos_x, self.pos_y}]', LOG_TAG))
