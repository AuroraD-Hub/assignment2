...
client = ArmorClient('client_build','assignment')
...
def count_markers():
	...
	if (tot_markers==pnm.TOTAL_MARKERS):
		...
		client.call('REMOVE','DATAPROP','IND',['now','Robot1','Long','1665579740'])
		client.call('ADD','DATAPROP','IND',['now','Robot1','Long',str(int(time.time()))])
		...
		#client.utils.save_ref_with_inferences(path + '/utils/prova.owl')
...
def build_map(map_info):
	...
	if (map_info.room == 'E') or (map_info.room == 'C1') or (map_info.room == 'C2'):
		...
		client.call('ADD','DATAPROP','IND',['visitedAt', map_info.room, 'Long', str(int(time.time()-1000))])
