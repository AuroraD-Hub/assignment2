...
def move_camera():
	...
	camera_pos = Float64()
	k_body = k_body+1
	rospy.loginfo(pnm.tag_log(f'k_body = {k_body}',pnm.NODE_BUILDER))
	if k_body>12: # camera is traslated up to detect markers on the walls
		camera_arm_pos = Float64()
		camera_arm_pos = 0.1
		pub_camera_arm.publish(camera_arm_pos)
		rospy.loginfo(pnm.tag_log(f'camera_arm_pos = {camera_arm_pos}',pnm.NODE_BUILDER))
		k_body = 0
	camera_pos = -3.14+k_body*0.5
	rospy.loginfo(pnm.tag_log(f'camera_pos = {camera_pos}',pnm.NODE_BUILDER))
