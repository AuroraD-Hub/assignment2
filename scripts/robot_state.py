...
class RobotState:
    ...
    def __init__(self):
        ...
        pub_posX = rospy.Publisher(f'{pnm.TOPIC_POSITION}/x', Float64, queue_size=1, latch=True)
        pub_posY = rospy.Publisher(f'{pnm.TOPIC_POSITION}/y', Float64, queue_size=1, latch=True)
        pos = Float64()
        pos.data = pnm.INITIAL_POSITION
        pub_posX.publish(pos[0])
        pub_posY.publish(pos[1])
